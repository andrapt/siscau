#!/usr/bin/env python
"""
Script para inserir dados de exemplo na aplicação usuarios
Executar com: python manage.py shell < inserir_dados_usuarios.py
"""
    ...
    with transaction.atomic():
        ...
        sistemas_data = [
            ...
            },
            {
                ...
                'url': 'http://localhost:8000/fazenda/',
                'ativo': True
            },
            {
                'nome': 'Fertilizantes',
                'descricao': 'Módulo de Controle de Fertilizantes',
                'url': 'http://localhost:8000/fertilizantes/',
                'ativo': True
            },
            {
                'nome': 'Financeiro',
        ...
        for sistema_data in sistemas_data:
            ...
            if created:
                print(f"  ✓ Sistema '{sistema.nome}' criado")
            else:
                print(f"  - Sistema '{sistema.nome}' já existe")
        ...
        for funcao_data in funcoes_data:
            ...
            if created:
                ...
                print(f"  - Função '{funcao.nome}' já existe")
        ...
        for perfil_data in perfis_data:
            ...
            if created:
                print(f"  ✓ Perfil '{perfil.nome}' criado para sistema '{perfil.sistema.nome}'")
            else:
                print(f"  - Perfil '{perfil.nome}' já existe para sistema '{perfil.sistema.nome}'")
        ...
        for acesso_data in acessos_data:
            ...
            if created:
                print(f"  ✓ Acesso criado: {usuario.username} -> {sistema.nome} ({perfil.nome})")
            else:
                print(f"  - Acesso já existe: {usuario.username} -> {sistema.nome} ({perfil.nome})")
    
    print("\n✅ Dados de exemplo criados com sucesso!")
    print("\n📊 Resumo:")
    print(f"  • Sistemas: {Sistema.objects.count()}")
    print(f"  • Funções: {Funcao.objects.count()}")
    print(f"  • Perfis: {Perfil.objects.count()}")
    print(f"  • Usuários: {User.objects.count()}")
    print(f"  • Acessos: {UsuarioSistema.objects.count()}")
    
    print("\n🔑 Credenciais de acesso:")
    print("  • admin / admin123 (Administrador)")
    print("  • gerente.fazenda / gerente123 (Gerente)")
    print("  • operador.campo / operador123 (Operador)")
    print("  • tecnico.fert / tecnico123 (Técnico)")
    print("  • consultor.fert / consultor123 (Consultor)")
    print("  • analista.fin / analista123 (Analista)")
